# COOL STUFF FOR COOL PEOPLE DOING COOL SCIENCE :sunglasses:

I got tired of bookmarking everything I found useful in my browser, so here's an organized list of stuff I think is useful/cool.

## TABLE OF CONTENTS

- [COOL STUFF FOR COOL PEOPLE DOING COOL SCIENCE :sunglasses:](#cool-stuff-for-cool-people-doing-cool-science-sunglasses)
  - [TABLE OF CONTENTS](#table-of-contents)
  - [GENERAL SCIENCE](#general-science)
  - [NGS](#ngs)
  - [GENERAL PROGRAMMING](#general-programming)
  - [BASH/CLI](#bash)
  - [PYTHON](#python)
  - [MACHINE LEARNING](#machine-learning)
  - [MATH](#math)

## GENERAL SCIENCE

- [Software Carpentry: I have no jokes, this is one of the most valuable resources I've ever come across](https://software-carpentry.org/lessons/)
- [Biorender: for when you want all the features of powerpoint without saying you used powerpoint](https://biorender.com)
- [Semantic Scholar: AI powered paper discovery, may or may not be useful just like all my ML models](https://www.semanticscholar.org)
- [XSEDE: Good way to get compute time if you don't have an HPC but need the resources](https://portal.xsede.org)
- [Inkscape: draw pretty pictures](https://inkscape.org)
- [How to Write a Scientific Review Article: wait you mean I have to __read__ the papers?](https://blog.addgene.org/how-to-write-a-scientific-review-article)
- [Overleaf: LaTex but fancier](https://www.overleaf.com)
- [An extremely clear and concise way to communicate when reviewing/commenting on something](https://conventionalcomments.org)
- [One of the best set of bulletpoints for effective team management I've seen](https://twitter.com/MrEchs/status/1516024322409832450)
- [Erik Anderson's lab has an excellent example "Dry Lab Guide"](http://andersenlab.org/dry-guide/2022-03-09/)

## NGS

- [Great list of textbooks for bioinformatics](http://crazyhottommy.blogspot.com/2019/09/my-opinionated-selection-of-booksurls.html)
- [EMBL Tutorials: lot of great walkthroughs, some specific to EMBL-EBI, some not](https://www.ebi.ac.uk/training/on-demand?facets=type:Online%20tutorial)
- [Biostars Tutorials: Did you know biostars isn't just a site for asking the same question about what the SAM bitflag field means for two decades?](https://www.biostars.org/t/tutorials/)
- [Biostars Handbook: Where you can read about what the SAM bitflag field means without asking some postdoc trying to procrastinate their work](https://www.biostars.org/p/225812/)
- [Genomics Workflows on AWS: Cloud is the future I guess](https://docs.opendata.aws/genomics-workflows/)
- [Snakemake: I like Snakemake how bout you, I like snakemake yes I do](https://snakemake.readthedocs.io/en/stable/)
- [More Snakemake](https://vincebuffalo.com/blog/2020/03/04/understanding-snakemake.html)
- [RNA Seq Workflow with Snakemake](http://pedagogix-tagc.univ-mrs.fr/courses/ABD/practical/snakemake/snake_intro.html)
- [Real Time Genomics Tools: Some neat simulation tools and other stuff](https://www.realtimegenomics.com/products/rtg-tools)
- [Mike Love RNA seq twitter thread](https://twitter.com/mikelove/status/1513468597288452097)
- [UCSC List of tools](https://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/)
- [Excellent workflow language tutorial](https://ricomnl.com/blog/bottom-up-bioinformatics-pipeline/)

## GENERAL PROGRAMMING

- [Docker: so Mac users can't complain about your software not working on their 2014 Air](https://www.docker.com)
- [A Guide to Good Research Code: you mean I can't just write 5 bash scripts and call it a day?](https://goodresearch.dev)
- [How to think like a computer scientist: lots of coffee](https://runestone.academy/ns/books/published/thinkcspy/index.html)
- [A Cloud Guru: Someone told me they use this to train newhires in industry for cloud computing](https://acloudguru.com)
- [VSCode Keyboard Shortcuts: now you can have emacs pinky without the pain of using emacs!](https://code.visualstudio.com/shortcuts/keyboard-shortcuts-windows.pdf)
- [github1s: Lets you use VSCode in a browser to see github repos](https://github1s.com)
- [ChooseALicense: do you really need something that isn't MIT?](https://choosealicense.com/licenses/)
- [Rstudio Cloud: waaaaaaaaaaay easier to use for classes than having everyone installing](https://login.rstudio.cloud/login?r)
- [Markdown2Pdf: pandoc? what's that?](http://markdown2pdf.com/index.html)

## BASH

- [Terminal-based delimited data tool](https://www.visidata.org)
- [Automation and Make: it's not just for C++!](https://swcarpentry.github.io/make-novice/)
- [Vim Cheat Sheet: so you don't get trapped in eternity at the Vim start page](https://vim.rtorr.com)
- [Regex Crossword: as if writing regex for NGS file headers wasn't enough fun](https://regexcrossword.com)
- [glogg: because sifting through logfiles is lame](https://glogg.bonnefon.org/index.html)
- [crontab guru: for when you really want to piss of sysadmins of your local HPC](https://crontab.guru)
- [Pandoc: a very useful document conversion tool](https://pandoc.org)

## PYTHON

- [PEP8: Python style guide](https://peps.python.org/pep-0008/#a-foolish-consistency-is-the-hobgoblin-of-little-minds)
- [black: an autoformatter for when you're too lazy to read PEP8](https://black.readthedocs.io/en/stable/)
- [Multiprocessing Guide: so you can speed up your time to OOM errors](https://www.machinelearningplus.com/python/parallel-processing-python/)
- [Conda: for when you _reeeeeeeeeeeeeeally_ need that python 2 package](https://conda.io/projects/conda/en/latest/index.html)
- [Dash: the Python equivalent to Rshiny](https://plotly.com/dash/)

## MACHINE LEARNING

- [ML Notebooks: Dang that's a good bunch of PyTorch tutorials](https://github.com/dair-ai/ML-Notebooks)
- [Python Data Science Handbook](https://github.com/jakevdp/PythonDataScienceHandbook)
- [Graph of classification accuracy metrics: Ever forget difference between FDR and FPR? Me too, this helps](https://en.wikipedia.org/wiki/Precision_and_recall#Definition_(classification_context))
- [Neural Network SVG Generator: please no more powerpoint NN figures](http://alexlenail.me/NN-SVG/LeNet.html)
- [Keras: the most top-level NN builder out there](https://keras.io/guides/functional_api/)
- [scikit-learn: no seriously, this is the most useful package in existence](https://scikit-learn.org/stable/)
- [9 Tools I wish I had learned before my PhD in Machine Learning: some of these are already on here, dang I'm good](https://towardsdatascience.com/nine-tools-i-wish-i-mastered-before-my-phd-in-machine-learning-708c6dcb2fb0)
- [DL2AI: One of the best intros to NN math I've seen](https://d2l.ai/chapter_preface/index.html)
- [Goodfellow Deep Learning Book: free math, and also free math](https://www.deeplearningbook.org)

## MATH

- [Art of Linear Algebra: because not all math is boring](https://github.com/kenjihiranabe/The-Art-of-Linear-Algebra)
- [Which statistical test should you choose: because unless you've read 20 textbooks some of these are gibberish](https://help.xlstat.com/6443-which-statistical-test-should-you-use)
- [Permutation testing interactive guide](https://www.jwilber.me/permutationtest/)
- [Univariate Distribution Relationships Chart: this got me through BCB720 homework](http://www.math.wm.edu/~leemis/chart/UDR/UDR.html)
- [Elements of statistical learning](https://hastie.su.domains/ElemStatLearn/)
- [Matrix Cookbook: tasty tasty matrices](https://www.math.uwaterloo.ca/~hwolkowi/matrixcookbook.pdf)
